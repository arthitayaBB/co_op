...
$query = "SELECT * FROM news WHERE N_id LIKE '%$search%' OR N_heading LIKE '%$search%' OR N_detail LIKE '%$search%'";
...
<body>
    ...
    <div class="content">
    <h2>ข้อมูลข่าวสาร</h2>
    <div class="d-flex justify-content-between mb-3"> <a href="add_news.php" class="btn btn-success">เพิ่มข้อมูลข่าวสาร</a> </div>
        ...
        <table id="newsTable" class="table table-bordered table-hover table-striped">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>
                            <a href="edit_news.php?id=<?php echo $row['N_id']; ?>" class="btn btn-warning btn-sm"><i class="fas fa-pencil-alt"></i>แก้ไข</a>
                            <a href="delete_news.php?id=<?php echo $row['N_id']; ?>" class="btn btn-danger btn-sm" onclick="return confirm('คุณแน่ใจหรือไม่?');"><i class="fas fa-trash-alt"></i>ลบ</a>
                        <td>
                            <?php 
                                if ($row['N_status'] == 1) {
                                    echo '<span class="text-success">อนุญาต</span>';
                                } else {
                                    echo '<span class="text-danger">ไม่อนุญาต</span>';
                                }
                            ?>
                        </td>
                        <td>
                            <img src="../images/<?php echo $row['N_picture']; ?>" alt="รูป" width="80" height="auto">
                        </td>
