<?php
include 'connectdb.php';
include 'check_admin.php';

if (isset($_POST['Submit'])) {
    // รับค่าจากฟอร์ม
    $Pr_title = mysqli_real_escape_string($conn, $_POST['Pr_title']);
    $Pr_detail = mysqli_real_escape_string($conn, $_POST['Pr_detail']);
    $company_id = mysqli_real_escape_string($conn, $_POST['company_id']);
    $Pr_status = mysqli_real_escape_string($conn, $_POST['Pr_status']);
    $admin_id = $_SESSION['Admin_id']; // ต้องมีการตั้ง session นี้ไว้ตอน login

    $Pr_date = date('Y-m-d');
    $Pr_year = date('Y') + 543;

    // อัปโหลดไฟล์
    $uploadDir = "../images/public_relations/";
    $allowedTypes = ['jpg', 'jpeg', 'png', 'gif'];
    $pictures = [null, null, null, null];

    if (!file_exists($uploadDir)) {
        mkdir($uploadDir, 0777, true); // สร้างโฟลเดอร์ถ้ายังไม่มี
    }

    // ตรวจสอบว่ามีไฟล์อัพโหลดหรือไม่
    if (isset($_FILES['pictures'])) {
        for ($i = 0; $i < count($_FILES['pictures']['name']); $i++) {
            if ($_FILES['pictures']['error'][$i] === UPLOAD_ERR_OK && $_FILES['pictures']['size'][$i] > 0) {
                $tmp_name = $_FILES['pictures']['tmp_name'][$i];
                $originalName = $_FILES['pictures']['name'][$i];
                $ext = strtolower(pathinfo($originalName, PATHINFO_EXTENSION));
        
                if (in_array($ext, $allowedTypes)) {
                    $timestamp = time();
                    $safeTitle = preg_replace("/[^a-zA-Z0-9ก-๙]/u", "_", $Pr_title); // ทำชื่อให้ปลอดภัย
                    $newName = $safeTitle . "_" . $timestamp . "-pr" . ($i+1) . "." . $ext;
        
                    $destination = $uploadDir . $newName;
                    if (move_uploaded_file($tmp_name, $destination)) {
                        $pictures[$i] = $newName;
                    }
                }
            }
        }
    }

    // บันทึกลงฐานข้อมูล
    $sql = "INSERT INTO public_relations 
        (Pr_picture1, Pr_picture2, Pr_picture3, Pr_picture4, Pr_title, Pr_detail, Company_id, Pr_date, Pr_year, Pr_status, Admin_id)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

    $stmt = $conn->prepare($sql);
    $stmt->bind_param(
        "ssssssisisi", // แก้ให้ตรงชนิดข้อมูล
        $pictures[0],  // s
        $pictures[1],  // s
        $pictures[2],  // s
        $pictures[3],  // s
        $Pr_title,     // s
        $Pr_detail,    // s
        $company_id,   // i
        $Pr_date,      // s
        $Pr_year,      // i
        $Pr_status,    // s
        $admin_id      // i
    );

    if ($stmt->execute()) {
        echo "<script>alert('บันทึกข้อมูลเรียบร้อยแล้ว'); window.location='indexactivity.php';</script>";
    } else {
        echo "<script>alert('เกิดข้อผิดพลาด: " . $stmt->error . "');</script>";
    }

    $stmt->close();
}
?>

<!DOCTYPE html>
<html lang="th">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>เพิ่มข้อมูลข่าวกิจกรรมสหกิจศึกษา</title>
    <link href="https://fonts.googleapis.com/css2?family=Noto+Sans+Thai:wght@400;700&display=swap" rel="stylesheet">
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
    <script src="https://cdn.jsdelivr.net/npm/particles.js"></script>
    <link rel="stylesheet" href="stylBEadd.CSS">
</head>

<body>
    <div class="container mt-5">
        <!-- ปุ่มกากบาทสำหรับกลับไปหน้าก่อน -->
        <button class="close-btn" onclick="window.history.back();">×</button>

        <h2 class="heading">เพิ่มข้อมูลข่าวกิจกรรมสหกิจศึกษา</h2>
        <form method="POST" enctype="multipart/form-data">
            <div class="mb-3 text-center">
                <label class="form-label">อัปโหลดรูปภาพ (สูงสุด 4 รูป)</label>
                <!-- พื้นที่แสดง preview -->
                <div id="previewContainer" class="mt-3 d-flex flex-wrap justify-content-center gap-2">
                    <p id="noImageText" class="text-muted">ไม่มีรูปภาพ</p>
                </div>
                <input type="file" class="form-control" id="pictureInput" name="pictures[]" accept="image/*" multiple>
            </div>

            <div class="form-group">
                <label for="Pr_title" class="form-label">หัวข้อ</label>
                <input type="text" name="Pr_title" id="Pr_title" class="form-control" required autofocus>
            </div>

            <div class="form-group">
                <label for="Pr_detail" class="form-label">รายละเอียด</label>
                <textarea name="Pr_detail" id="Pr_detail" class="form-control" rows="4" required autofocus></textarea>
            </div>

            <?php
            $sql = "SELECT * FROM company ORDER BY Company_id ASC";
            $result = mysqli_query($conn, $sql);

            // สร้าง array สำหรับจับคู่ชื่อกับ ID (JS ต้องใช้)
            $companies = [];
            while ($row = mysqli_fetch_assoc($result)) {
                $companies[] = [
                    'id' => $row['Company_id'],
                    'name' => $row['NamecomTH']
                ];
            }
            ?>

            <div class="col-md- mb-3">
                <label class="form-label">
                    <i class="bi bi-briefcase-fill me-2" style="color: skyblue;"></i>พิมพ์หรือเลือกบริษัท
                </label>

                <!-- ผู้ใช้พิมพ์ชื่อบริษัท -->
                <input type="text" class="form-control" list="companyList" id="company_name_input" oninput="matchCompany()" required>

                <!-- ค่า Company_id ที่ซ่อน -->
                <input type="hidden" name="company_id" id="company_id_hidden">

                <!-- datalist แสดงชื่อบริษัท -->
                <datalist id="companyList">
                    <?php foreach ($companies as $c): ?>
                        <option value="<?= htmlspecialchars($c['name']) ?>"></option>
                    <?php endforeach; ?>
                </datalist>
            </div>

            <div class="form-group">
                <label for="Pr_status" class="form-label">Status</label>
                <select name="Pr_status" id="Pr_status" class="form-control" required>
                    <option value="0">ไม่แสดง</option>
                    <option value="1">แสดง</option>
                </select>
            </div>

            <div class="form-group text-center">
                <button type="submit" name="Submit" class="btn btn-primary">บันทึกข้อมูล</button>
            </div>
        </form>
    </div>

    <script>
        // สร้าง Map ชื่อ -> ID (จาก PHP -> JS)
        const companyMap = <?= json_encode(array_column($companies, 'id', 'name')) ?>;

        function matchCompany() {
            const nameInput = document.getElementById('company_name_input').value.trim();
            const hiddenIdInput = document.getElementById('company_id_hidden');

            if (companyMap.hasOwnProperty(nameInput)) {
                hiddenIdInput.value = companyMap[nameInput];
            } else {
                hiddenIdInput.value = '';
            }
        }

        // แสดง preview รูปภาพ
        const pictureInput = document.getElementById('pictureInput');
        const previewContainer = document.getElementById('previewContainer');
        const noImageText = document.getElementById('noImageText');

        pictureInput.addEventListener('change', function(event) {
            previewContainer.innerHTML = '';
            
            const files = event.target.files;
            
            if (files.length === 0) {
                noImageText.style.display = 'block';
            } else {
                noImageText.style.display = 'none';
                
                // จำกัดจำนวนไฟล์ที่แสดงเป็น 4 ไฟล์
                const maxFiles = Math.min(files.length, 4);
                
                for (let i = 0; i < maxFiles; i++) {
                    const file = files[i];
                    const reader = new FileReader();
                    
                    reader.onload = function(e) {
                        const imgWrapper = document.createElement('div');
                        imgWrapper.className = 'position-relative';
                        
                        const img = document.createElement('img');
                        img.src = e.target.result;
                        img.className = 'img-thumbnail';
                        img.style.width = '120px';
                        img.style.margin = '5px';
                        
                        imgWrapper.appendChild(img);
                        previewContainer.appendChild(imgWrapper);
                    }
                    
                    reader.readAsDataURL(file);
                }
            }
        });
    </script>
    <script src="scriptBEadd.js"></script>
</body>
</html>